...
function getRandomPokemon(list) {
  ...
  const url = list[randomIndex].url;
...
// ✅ Display Pokémon in player's card
...
// ✅ Main fight function using ONLY PROMISES
function fight() {
  getPokemonList()
    .then((list) => {
      // fetch 2 random Pokémon at the same time
      return Promise.all([getRandomPokemon(list), getRandomPokemon(list)]);
    })
    .then(([p1, p2]) => {
      // Display on screen
      displayPokemon(
